...
proxies, TIMEOUT_SECONDS, MAX_RETRY, DEEPSEEK_API_KEY = \
    get_conf('proxies', 'TIMEOUT_SECONDS', 'MAX_RETRY', 'DEEPSEEK_API_KEY')
    ...
    while True:
        ...
        if not chunk_decoded.startswith('data:'):
            ...
            if "reduce the length" in error_msg:
                raise ConnectionAbortedError("DEEPSEEK拒绝了请求:" + error_msg)
            elif """type":"upstream_error","param":"307""" in error_msg:
                raise ConnectionAbortedError("正常结束，但显示Token不足，导致输出不完整，请削减单次输入的文本量。")
            else:
                raise RuntimeError("DEEPSEEK拒绝了请求：" + error_msg)
        if ('data: [DONE]' in chunk_decoded): break # DEEPSEEK 正常完成
        # 提前读取一些信息 （用于判断异常）
...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream = True, additional_fn=None):
    ...
    if len(DEEPSEEK_API_KEY) == 0:
        raise RuntimeError("没有设置DEEPSEEK_API_KEY选项")
    ...
    if stream:
        ...
        while True:
            try:
                ...
                # 非OpenAI官方接口的出现这样的报错，OpenAI和DEEPSEEK不会走这里
                chunk_decoded = chunk.decode()
            ...
            if chunk:
                try:
                    ...
                    # 前者是DEEPSEEK的结束条件，后者是OPENAI的结束条件
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    """
    api_key = f"Bearer {DEEPSEEK_API_KEY}"
    ...
    if llm_kwargs['llm_model'] == "deepseek-chat":
        tokens = 32000
    else: # deepseek-coder
        tokens = 16000
