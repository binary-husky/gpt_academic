...
def predict(inputs: str, llm_kwargs: dict, plugin_kwargs: dict, chatbot: ChatBotWithCookies,
    ...
    # Print initial inputs for debugging
    print(f"Inputs: {inputs}")
    print(f"llm_kwargs: {llm_kwargs}")
    print(f"plugin_kwargs: {plugin_kwargs}")
    ...
    # Add max_tokens to llm_kwargs if not already present
    if 'max_tokens' not in llm_kwargs:
        llm_kwargs['max_tokens'] = 1024 * 4

    # Check if model has multimodal capacity
    ...
    # Disable streaming for certain models if needed
    ...
    while True:
        try:
            ...
            yield from update_ui(chatbot=chatbot, history=history, msg="请求失败" + retry_msg)
            if retry > MAX_RETRY:
