...
from toolbox import get_conf, update_ui, trimmed_format_exc, is_the_upload_folder, log_chat
proxies, TIMEOUT_SECONDS, MAX_RETRY, GROQ_API_KEY = \
    get_conf('proxies', 'TIMEOUT_SECONDS', 'MAX_RETRY', 'GROQ_API_KEY')
    ...
    while True:
        ...
        if not chunk_decoded.startswith('data:'):
            ...
            if "reduce the length" in error_msg:
                raise ConnectionAbortedError("GROQ拒绝了请求:" + error_msg)
            elif """type":"upstream_error","param":"307""" in error_msg:
                raise ConnectionAbortedError("正常结束，但显示Token不足，导致输出不完整，请削减单次输入的文本量。")
            else:
                raise RuntimeError("GROQ拒绝了请求：" + error_msg)
        if ('data: [DONE]' in chunk_decoded): break # GROQ 正常完成
        # 提前读取一些信息 （用于判断异常）
...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream = True, additional_fn=None):
    ...
    if len(GROQ_API_KEY) == 0:
        raise RuntimeError("没有设置GROQ_API_KEY选项")
    ...
    if stream:
        ...
        while True:
            try:
                ...
                # 非OpenAI官方接口的出现这样的报错，OpenAI和GROQ不会走这里
                chunk_decoded = chunk.decode()
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    if llm_kwargs['llm_model'] in ['groq-llama3-8b-8192', 'groq-llama3-70b-8192', 'groq-gemma-7b-it']:
        tokens = 8192
    elif llm_kwargs['llm_model'] == 'groq-llama2-70b-4096':
        tokens = 4096
    else: # 'groq-mixtral-8x7b-32768'
        tokens = 32768

    payload = {
        "model": model.removeprefix('groq-'),
        "messages": messages,
        "temperature": 0.01 if llm_kwargs['temperature'] == 0 else llm_kwargs['temperature'], # groq converts a temperature value of 0 to 1e-8.
        "stream": stream,
        "max_tokens": tokens
    }
    try:
