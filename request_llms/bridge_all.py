    ...
    if "&" not in model:
        ...
        def mutex_manager(window_mutex, observe_window):
            while True:
                ...
                for i in range(n_model):
                    ...
                    chat_string.append(
                        f'【{str(models[i])} 说】: <font color="{color}"> {window_mutex[i][0]} </font>'
                    )
                res = "<br/><br/>\n\n---\n\n".join(chat_string)
                # # # # # # # # # # #
        ...
        for i, future in enumerate(futures):  # wait and get
            ...
            return_string_collect.append(
                f'【{str(models[i])} 说】: <font color="{color}"> {future.result()} </font>'
            )
...
def predict(
    ...
    if llm_kwargs["llm_model"] not in model_info:
        ...
        chatbot.append(
            [
                inputs,
                f"很抱歉，模型 '{llm_kwargs['llm_model']}' 暂不支持<br/>(1) 检查config中的AVAIL_LLM_MODELS选项<br/>(2) 检查request_llms/bridge_all.py中的模型路由",
            ]
        )
