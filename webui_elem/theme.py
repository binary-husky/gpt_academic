...
def dynamic_set_theme(THEME):
    ...
    with ProxyNetworkActivate('Download_Gradio_Theme'):
        ...
        try:
            ...
            logging.warning('下载主题失败')
    return set_theme


def chuanhu_adjust_theme():
...
def load_dynamic_theme(THEME):
    ...
    if THEME == 'Chuanhu-Plus':
        adjust_theme = chuanhu_adjust_theme()
    elif '/' in THEME:
        adjust_dynamic_theme = dynamic_set_theme(THEME)
        adjust_theme = adjust_dynamic_theme
    else:
        adjust_theme = chuanhu_adjust_theme()
    return adjust_theme, adjust_dynamic_theme


adjust_theme, _ = load_dynamic_theme(AVAIL_THEMES[0])
