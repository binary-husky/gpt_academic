...
def main():
    ...
    with gr.Blocks(title="GPT 学术优化", theme=set_theme, analytics_enabled=False, css=advanced_css) as demo:
        gr.HTML(title_html)
        secret_css, dark_mode, py_pickle_cookie = gr.Textbox(visible=False), gr.Textbox(DARK_MODE,
        ...
        with gr.Floating(init_x="0%", init_y="0%", visible=True, width=None, drag="forbidden", elem_id="tooltip"):
            with gr.Row():
                ...
                with gr.Tab("界面外观", elem_id="interact-panel"):
                    ...
                    checkboxes_2 = gr.CheckboxGroup(opt, value=value, label="显示/隐藏自定义菜单",
                                                    elem_id='cbsc').style(container=False)
            ...
            with gr.Accordion("浮动输入区", open=True, elem_id="input-panel2"):
                with gr.Row() as row:
                    ...
                    with gr.Column(scale=1, min_width=40):
                        ...
                        clearBtn2 = gr.Button("清除", elem_id="elem_clear2", variant="secondary", visible=False);
                ...
                with gr.Row() as row:
                    ...
                    with gr.Column(scale=1, min_width=70):
                        ...
                        def assign_btn(persistent_cookie_, cookies_, basic_btn_dropdown_, basic_fn_title,
                                       basic_fn_prefix, basic_fn_suffix, clean_up=False):
                            ret = {}
                            # 读取之前的自定义按钮
                            ...
                            if basic_btn_dropdown_ in customize_btns:
                                # 是自定义按钮，不是预定义按钮
                                ret.update({customize_btns[basic_btn_dropdown_]: gr.update(visible=visible,
                                                                                           value=basic_fn_title)})
                            else:
                                # 是预定义按钮
                                ret.update({predefined_btns[basic_btn_dropdown_]: gr.update(visible=visible,
                                                                                            value=basic_fn_title)})
                        ...
                        h = basic_fn_confirm.click(assign_btn,
                                                   [py_pickle_cookie, cookies, basic_btn_dropdown, basic_fn_title,
                                                    basic_fn_prefix, basic_fn_suffix],
                                                   [py_pickle_cookie, cookies, *customize_btns.values(),
                                                    *predefined_btns.values()])
                        h.then(None, [py_pickle_cookie], None,
                               _js="""(py_pickle_cookie)=>{setCookie("py_pickle_cookie", py_pickle_cookie, 365);}""")
                        # clean up btn
                        h2 = basic_fn_clean.click(assign_btn,
                                                  [py_pickle_cookie, cookies, basic_btn_dropdown, basic_fn_title,
                                                   basic_fn_prefix, basic_fn_suffix, gr.State(True)],
                                                  [py_pickle_cookie, cookies, *customize_btns.values(),
                                                   *predefined_btns.values()])
                        h2.then(None, [py_pickle_cookie], None,
                                _js="""(py_pickle_cookie)=>{setCookie("py_pickle_cookie", py_pickle_cookie, 365);}""")
        ...
        demo.load(persistent_cookie_reload, inputs=[py_pickle_cookie, cookies],
                  outputs=[py_pickle_cookie, cookies, *customize_btns.values(), *predefined_btns.values()],
                  _js=js_code_for_persistent_cookie_init)
        demo.load(None, inputs=[dark_mode], outputs=None,
                  _js="""(dark_mode)=>{apply_cookie_for_checkbox(dark_mode);}""")  # 配置暗色主题或亮色主题
    ...
    demo.auth = AUTHENTICATION if len(AUTHENTICATION) != 0 else None
    demo.blocked_paths = ["config.py", "config_private.py", "docker-compose.yml", "Dockerfile",
                          f"{PATH_LOGGING}/admin"]
    return demo, PORT


from shared_utils.api_server import create_app
...
gr.mount_gradio_app(app, gr_demo, CUSTOM_PATH)

if __name__ == "__main__":
    import uvicorn

    config = uvicorn.Config("__main__:app", host="0.0.0.0", port=PORT, reload=False)
