from common.gr_converter_html import html_local_img
...
def 图片生成_DALLE2(prompt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    markd_show = to_markdown_tabs(head=['图像预览'],
                                           tabs=[[html_local_img(image_path),
...
class ImageEditState(GptAcademicState):
    ...
    def reset(self):
        self.req = [
            {'value': None, 'description': '请先上传图像（必须是.png格式）, 然后再次点击本插件',
             'verify_fn': self.get_image_file},
            {'value': None, 'description': '请输入分辨率,可选：256x256, 512x512 或 1024x1024, 然后再次点击本插件',
             'verify_fn': self.get_resolution},
            {'value': None, 'description': '请输入修改需求,建议您使用英文提示词, 然后再次点击本插件',
             'verify_fn': self.get_prompt},
        ]
        self.info = ""
