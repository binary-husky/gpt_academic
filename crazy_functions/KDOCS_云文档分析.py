...
def Kdocs_多阶段生成回答(user_input, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    for stage in multi_stage_config:
        ...
        embedding_limit = yield from crazy_box.func_拆分与提问(embedding_limit, llm_kwargs, plugin_kwargs, chatbot, history,
                                                          plugin_prompt=prompt, knowledge_base=knowledge)
        if func and func_kwargs.get(func, False):
            gpt_results_count[prompt] = yield from func_kwargs[func](embedding_limit, llm_kwargs, plugin_kwargs, chatbot, history)
            embedding_limit = []
        else:
        ...
        if stage != [i for i in multi_stage_config][-1]:
            yield from crazy_box.file_extraction_intype(gpt_results_count[prompt], chatbot, history, llm_kwargs, plugin_kwargs)
