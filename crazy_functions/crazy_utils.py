        ...
        for index, page in enumerate(doc):
            ...
            for t in text_areas['blocks']:
                if 'lines' in t:
                    ...
                    os.makedirs(pdf_save_img_path, exist_ok=True)
...
if __name__ == '__main__':
        ...
        with fitz.open(fp) as doc:
            ...
            for index, page in enumerate(doc):
                ...
                for t in text_areas['blocks']:
                    ...
                    #         pdf_save_img_path = save_path
                    #     else:
                    #         pdf_save_img_path = os.path.join(init_path.private_files_path, 'temp_file', 'pdf_img',
                    #                                          str(doc.name).split('.')[0])
                    #     os.makedirs(pdf_save_img_path, exist_ok=True)
                    #     bbox_tag = "-".join([str(int(i)) for i in t["bbox"]])
                    #     save_file_name = os.path.join(pdf_save_img_path, f'{index}-{t["size"]}_{bbox_tag}.{t["ext"]}')
                    #     with open(save_file_name, 'wb') as f:
                ...
                if index == 0:
                    page_one_meta = [
                        " ".join(["".join([wtf['text'] for wtf in l['spans']]) for l in t['lines']]).replace(
                            '- ', '') for t in text_areas['blocks'] if 'lines' in t]
    ...
    text, lis =  read_and_clean_pdf_text('/Users/kilig/Documents/PDFbatch/罗宾斯第13版重点.pdf')
    print(text)
    print(lis)
