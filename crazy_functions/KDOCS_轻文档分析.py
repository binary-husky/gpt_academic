...
def Kdocs_轻文档批量处理(link_limit, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    for url in links:
        try:
            ...
            yield from toolbox.update_ui(chatbot, history)  # 增加中间过渡
...
def KDocs_转Markdown(link_limit, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if not file_limit:
        ...
        yield from toolbox.update_ui(chatbot=chatbot, history=history, msg='无法获取需求文档内容，暂停运行')
        return
...
def KDocs_转接口测试用例(file_limit, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if not gpt_response_collection:
        ...
        yield from toolbox.update_ui(chatbot=chatbot, history=history, msg='多线程一个都没有通过，暂停运行')
        return
    ...
    yield from toolbox.update_ui(chatbot, history, '插件执行成功')
