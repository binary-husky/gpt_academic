...
def for_immediate_show_off_when_possible(file_type, fp, chatbot):
    if file_type in valid_img_extensions:
...
def 虚空终端CodeInterpreter(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    # 清空历史，以免输入溢出
    history = [];
    clear_file_downloadzone(chatbot)
    ...
    # 读取文件
    if ("recently_uploaded_files" in plugin_kwargs) and (
            plugin_kwargs["recently_uploaded_files"] == ""): plugin_kwargs.pop("recently_uploaded_files")
