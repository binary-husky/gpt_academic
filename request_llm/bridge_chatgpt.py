...
def predict_no_ui_long_connection(inputs, llm_kwargs, history=[], sys_prompt="", observe_window=None, console_slience=False):
    ...
    if json_data['finish_reason'] == 'content_filter':
        raise RuntimeError(f"raise ConnectionAbortedError jsokf 由于提问含不合规内容被Azure过滤。\n\n{result}\n\n")
...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream=True, additional_fn=None):
    ...
    yield from toolbox.update_ui(chatbot=chatbot, history=history, msg="等待响应") # 刷新界面
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    if llm_kwargs['llm_model'].startswith('proxy-'):
        headers.update({"api-key": f"{api_key}"})
