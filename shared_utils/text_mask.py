...
def apply_gpt_academic_string_mask(string, mode="show_all"):
    ...
    """
    if_string = str(string)
    if "<gpt_academic_string_mask>" not in if_string: # No need to process
        return string

    if mode == "show_all":
        return string
    if mode == "show_llm":
        string = const_extract_re.sub(r"\1", if_string)
    elif mode == "show_render":
        string = const_extract_re.sub(r"\2", if_string)
    else:
